...
export function PictureThumb({ fileId, title, onOpen }: PictureThumbProps) {
  ...
  return (
    <button
      ...
      type="button"
      onClick={onOpen}
      className="group relative block aspect-square w-full overflow-hidden bg-[#ebe4d8] text-left focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#C8A24D] focus-visible:ring-offset-2"
      ...
      {url && !failed ? (
        ...
        <div className="flex h-full w-full flex-col items-center justify-center gap-2 text-[#8a8478]">
          ...
          <span className="text-xs">
            {failed ? "Vorschau fehlgeschlagen" : loading || visible ? "Lädt…" : "—"}
          </span>
        </div>
      )}
